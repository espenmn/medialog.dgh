...
class IMailToGroupAction(Interface):
...
@implementer(IMailToGroupAction, IRuleElementData)
class MailToGroupAction(SimpleItem):
    ...
    element = 'plone.actions.MailToGroup'
...
@adapter(Interface, IMailToGroupAction, Interface)
...
class MailToGroupAddForm(ActionAddForm):
...
class MailToGroupAddFormView(ContentRuleFormWrapper):
    form = MailToGroupAddForm


class MailToGroupEditForm(ActionEditForm):
    """ An edit form for the mail group action """
    schema = IMailToGroupAction
